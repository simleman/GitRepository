...
<body>
	...
	<c:if test="${not empty paperList.station.name}">
		<div style="position: absolute; left: 50%; margin: 0 0 0 -180px;">岗位要求：${paperList.station.name}</div>
	</c:if>
	<c:if test="${empty paperList.station.name}">
		<div style="position: absolute; left: 50%; margin: 0 0 0 -180px;">岗位要求：混合岗位</div>
	</c:if>
	...
	<a href="<%=request.getContextPath()%>/printpaperview.jsp">day</a>
	</div>
</body>
</html>